...
abstract class AdvantageDao : BaseDao<Advantage> {
    ...
    @Query("SELECT advantageId FROM charactersheetadvantagecrossref WHERE characterSheetId=:characterSheetId")
    abstract fun getIdsAddedToCharacterSheet(characterSheetId: Long): Flow<List<Long>>
}